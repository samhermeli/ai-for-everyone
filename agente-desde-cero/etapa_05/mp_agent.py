"""
Agente Mercado Pago — version 04 (con soporte de variantes).

Diferencia clave vs 03_evals/mp_agent.py:
  - Introduce AgentVariant: una variante es una version del agente con prompt distinto
  - run_agent acepta system_prompt como parametro para permitir variantes
  - 3 variantes predefinidas: baseline, with_synonyms, direct_no_confirmation
  - start_main_span_with_variant taggea el span raiz con variant.name

NO contiene evals, datasets ni evaluadores — eso vive en mp_evals.py,
mp_convergence.py y mp_experiments.py.
...
# ---------------------------------------------------------------------------
# Tool: create_payment_link (idéntica a 03)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# System prompts — uno por variante
# ---------------------------------------------------------------------------
...
SYSTEM_PROMPT_WITH_SYNONYMS = SYSTEM_PROMPT_BASELINE + """

IMPORTANTE — Activacion de la tool con sinonimos:
Los siguientes verbos del usuario activan create_payment_link (todos significan lo mismo):
  - "cobrar", "cobrame", "necesito cobrar"
  - "vender", "vendo", "quiero vender"
  - "generar pago", "generar cobro", "generar un link"
  - "hacer factura", "hacer una venta", "hacer un cobro"
  - "crear link", "crear preferencia"

Si el usuario usa cualquiera de estos verbos Y proporciona titulo + precio,
debes llamar la tool inmediatamente sin pedir confirmacion.
"""
...
Llama create_payment_link en cuanto tengas suficiente informacion. NO confirmes,
NO repreguntes datos ya dados, NO expliques lo que vas a hacer.

Parametros:
  - title: el producto mencionado (extraelo del mensaje, infiere si es necesario)
  - quantity: cantidad explicita; si no se menciona, usa 1
  - unit_price: precio mencionado, convertido a numero

Si falta uno de los tres datos, pregunta SOLO por el faltante, en una sola
pregunta corta. Sin saludos ni preambulos.

Al entregar el link, devuelve solo el sandbox_init_point en una linea.
Responde en el idioma del usuario.
"""


# ---------------------------------------------------------------------------
# Variantes — cada una es una version del agente
# ---------------------------------------------------------------------------
@dataclass(frozen=True)
class AgentVariant:
    """Una variante del agente — el agente parametrizado.

    Por ahora solo se varia el system_prompt. En el futuro podrian agregarse
    otros campos: model, temperature, tool_descriptions, etc.
    """
    name: str
    system_prompt: str


VARIANT_BASELINE = AgentVariant(
    name="baseline",
    system_prompt=SYSTEM_PROMPT_BASELINE,
)

VARIANT_WITH_SYNONYMS = AgentVariant(
    name="with_synonyms",
    system_prompt=SYSTEM_PROMPT_WITH_SYNONYMS,
)

VARIANT_DIRECT = AgentVariant(
    name="direct_no_confirmation",
    system_prompt=SYSTEM_PROMPT_DIRECT,
)

ALL_VARIANTS = [VARIANT_BASELINE, VARIANT_WITH_SYNONYMS, VARIANT_DIRECT]
...
def run_agent(
    ...
    """
    Router loop con max_iterations. Acepta system_prompt como parametro
    para permitir correr distintas variantes del agente sin duplicar codigo.
    """
    messages = [
...
def start_main_span(user_message: str, variant: AgentVariant = VARIANT_BASELINE) -> str:
    """Punto de entrada con span raiz tipo agent.

    El variant.name se guarda como atributo del span — asi al inspeccionar la
    traza sabemos que version del agente la genero.
    """
    with tracer.start_as_current_span("AgentRun") as span:
        ...
        try:
            result = run_agent(user_message, system_prompt=variant.system_prompt)
...
# ---------------------------------------------------------------------------
# Modo standalone — chat con variante baseline
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    print("\n=== Agente Mercado Pago — variante baseline ===")
    print("Para probar otras variantes use mp_experiments.py")
    ...
    while True:
        ...
        if not user_input:
            continue
        try:
            result = start_main_span(user_input, variant=VARIANT_BASELINE)
            print(f"\nAgente: {result}\n")
